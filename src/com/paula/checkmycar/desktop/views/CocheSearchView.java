...
public class CocheSearchView extends View {
	...
	private void initialize() {
		...
		c.gridx = 0;
		c.gridy = y;
		criteriosPanel.add(new JLabel("Matrícula:"), c);
		matriculaTF = new JTextField();
		c.gridx = 1;
		criteriosPanel.add(matriculaTF, c);

		c.gridx = 2;
		criteriosPanel.add(new JLabel("Nº Bastidor:"), c);
		numeroBastidorTF = new JTextField();
		c.gridx = 3;
		criteriosPanel.add(numeroBastidorTF, c);

		y++;
		c.gridx = 0;
		c.gridy = y;
		criteriosPanel.add(new JLabel("Marca:"), c);
		marcaComboBox = new JComboBox<>();
		c.gridx = 1;
		criteriosPanel.add(marcaComboBox, c);

		c.gridx = 2;
		criteriosPanel.add(new JLabel("Modelo:"), c);
		modeloComboBox = new JComboBox<>();
		c.gridx = 3;
		criteriosPanel.add(modeloComboBox, c);

		y++;
		c.gridx = 0;
		c.gridy = y;
		criteriosPanel.add(new JLabel("Combustible:"), c);
		tipoCombustibleComboBox = new JComboBox<>();
		c.gridx = 1;
		criteriosPanel.add(tipoCombustibleComboBox, c);

		c.gridx = 2;
		criteriosPanel.add(new JLabel("Motor:"), c);
		tipoMotorComboBox = new JComboBox<>();
		c.gridx = 3;
		criteriosPanel.add(tipoMotorComboBox, c);

		y++;
		c.gridx = 0;
		c.gridy = y;
		criteriosPanel.add(new JLabel("Transmisión:"), c);
		tipoTransmisionComboBox = new JComboBox<>();
		c.gridx = 1;
		criteriosPanel.add(tipoTransmisionComboBox, c);

		y++;
		c.gridx = 0;
		c.gridy = y;
		criteriosPanel.add(new JLabel("Precio mín:"), c);
		precioMinTF = new JTextField();
		c.gridx = 1;
		criteriosPanel.add(precioMinTF, c);

		c.gridx = 2;
		criteriosPanel.add(new JLabel("Precio máx:"), c);
		precioMaxTF = new JTextField();
		c.gridx = 3;
		criteriosPanel.add(precioMaxTF, c);
	...
	private void postInitialize() {
		...
		try {
			...
			for (TipoCombustible tc : tipoCombustibleService.findAll()) {
			...
			for (TipoMotor tm : tipoMotorService.findAll()) {
			...
			for (TipoTransmision tt : tipoTransmisionService.findAll()) {
			...
			JOptionPane.showMessageDialog(this, "Error al cargar los filtros.", "Error", JOptionPane.ERROR_MESSAGE);
		...
		resultadosTable.getColumnModel().getColumn(4).setCellRenderer(new ClienteEmpleadoButtonRenderer());

		resultadosTable.getColumnModel().getColumn(4).setCellEditor(new CocheButtonEditor(new JCheckBox()));
	...
	private void buscarPagina(int pagina) {
		...
		if (transmision != null && transmision.getId() != null) {
			criteria.setTipoTransmisionId(transmision.getId());
		}
		...
		try {
			...
			if (!precioMin.isEmpty()) {
				criteria.setPrecioMin(Double.parseDouble(precioMin));
			}

			String precioMax = precioMaxTF.getText().trim();

			if (!precioMax.isEmpty()) {
				criteria.setPrecioMax(Double.parseDouble(precioMax));
			}

		} catch (NumberFormatException e) {

			JOptionPane.showMessageDialog(this, "El precio introducido no es válido.", "Error",
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Error al buscar coches.", "Error", JOptionPane.ERROR_MESSAGE);
